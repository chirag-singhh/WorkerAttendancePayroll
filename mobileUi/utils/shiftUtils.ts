...
export const SHIFT_OPTIONS = [
  ...
  { value: 0.5,  label: 'P½',   display: 'Half Shift' },
  { value: 1,    label: 'P',    display: 'Present' },
  { value: 1.5,  label: '1.5P', display: '1.5 Shifts' },
 ...
 */
export function getShiftColorClass(value: any) {
  const num = parseFloat(value);
  if (isNaN(num) || num === 0) return 'shift-absent';
  if (num === 0.5) return 'shift-half';
 ...
 */
export function getShiftBadgeStyle(value: any) {
  const num = parseFloat(value);
  if (isNaN(num) || num === 0) return { bg: '#fef2f2', text: '#b91c1c', border: '#fecaca' };
  if (num === 0.5) return { bg: '#eff6ff', text: '#1d4ed8', border: '#bfdbfe' };
